...
const getApiInfo = async () => {
  ...
  // console.log(apiUrl);      // https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&number=100&addRecipeInformation=true
...
async function getAallRecipes(req, res) {
  ...
  if (!name) {  // si no viene ningun nombre entra al if
    try {
      ...
      // una vez que terminan todas la promesas, le pido que concatene todas la recetas
    ...
    const query = name.toLowerCase(); // hago que el nombre lo pase todo a 
    // minuscula , asi no tengo problemas mas adelante para filtrar
    try {
 ...
 // si el titulo de la receta que traigo desde la api , incluye el nombre que me pasaron por params 
    ...
    // op(funcion de sql) --> va a filtrar si encuentra algun titulo parecido al nombre que me pasan por query 
      ...
      // una vez que terminan todas la promesas , concatenamos las dos informaciones
      console.log(respuesta.length);
      if (respuesta.length === 0) res.send(await getAllRecipes()) 
      // si no matcheo ninguna de las dos, es decir que no esxiste el nombre que me pasaron lor query
